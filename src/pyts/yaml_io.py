...
def read_yaml_stream(stream: TextIO) -> Any:
    """Read one YAML document from an already-open text stream.

    The caller owns the stream and is responsible for closing it. Parsing uses
    ``yaml.safe_load`` with the same return behavior as ``read_yaml``.
    """
