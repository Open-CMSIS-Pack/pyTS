...
class CoreSightEncoder(Protocol):
    """Stateless architecture-specific data request encoder."""

    def encode_data(
        self,
        request: DataTraceRequest,
        allocator: ComparatorAllocator,
    ) -> list[RegisterWrite]:
        """Encode a request with comparator state supplied by the caller."""
...
class CoreSight(ABC):
    ...
    def data_registers(
        ...
        access: str,
        output: str,
        match: DataMatch | None,
    ) -> list[YamlMapping]:
        """Compatibility adapter for callers supplying validated primitives."""

        request = DataTraceRequest(
            address=address,
            size=size,
            access=DataAccess(access),
            output=DataOutput(output),
            match=match,
        )
        return self.encode_data(request)
