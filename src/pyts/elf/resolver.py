...
class ElfResolver:
    ...
    def __init__(
        ...
        *,
        elf_file: ELFLike | None = None,
    ) -> None:
        """Create a resolver for exactly one path or caller-owned ELF object."""

        if (elf_path is None) == (elf_file is None):
            raise ValueError("provide exactly one of elf_path or elf_file")
        self.elf_path = Path("<memory>") if elf_path is None else Path(elf_path)
    ...
    @classmethod
    def from_path(cls, elf_path: str | Path) -> ElfResolver:
        """Create a lazily opened resolver for a filesystem path."""

        return cls(elf_path)

    @classmethod
    def from_elf(cls, elf_file: ELFLike) -> ElfResolver:
        """Create a resolver for a caller-owned, already-open ELF object."""

        return cls(elf_file=elf_file)
    ...
    def resolve_object_members(
        self,
        expressions: Sequence[str],
        *,
        source_file: str | None = None,
    ) -> list[MemberInfo]:
        """Resolve dotted object-member expressions through DWARF metadata."""

        self._check_open()
        return self._dwarf().resolve_members(
            expressions,
