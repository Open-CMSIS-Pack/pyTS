...
from pyts.coresight.model import CoreSight, DataMatch
...
def _patch_trace_resolver(
    ...
    *,
    ...
    monkeypatch.setattr("pyts.trace.workflow.ElfResolver", FakeElfResolver)
...
def test_direct_dwtv1_match_requires_pair_before_plain_allocation() -> None:
    ...
    first = coresight.data_registers(0x20000000, 4, "W", "value", None)
    with pytest.raises(ValueError, match="requires comparators 0 and 1"):
        coresight.data_registers(
            0x20000004,
            4,
            "W",
            "value",
            DataMatch(value=1, size=4),
        )
    second = coresight.data_registers(0x20000008, 4, "W", "value", None)
...
def test_invalid_coresight_request_does_not_consume_comparators() -> None:
    ...
    with pytest.raises(ValueError, match="cannot emit an address"):
        coresight.data_registers(0x20000000, 1, "W", "address", None)

    assert coresight.comparators.next_index == 0
    registers = coresight.data_registers(0x20000000, 4, "W", "value", None)
...
def test_trace_transformation_does_not_mutate_source_document() -> None:
    ...
    class FakeResolver:
        ...
        def resolve_symbols(
            ...
            return [_main_symbol()] if names and "main" in names else []
    ...
    def resolver_factory(_path: Path) -> Any:
        return FakeResolver()

    with SymbolCatalog(
        [SymbolFile(Path("fixture.elf"), frozenset(), None)],
        resolver_factory=resolver_factory,
    ) as catalog:
        result = transform_trace_document(cast(Any, source), catalog)
